...
public class ConnectionSettings extends PreferenceActivity {
    ...
    private void createDynamicSettings(final String keyPrefix,
        ...
        final ListPreference prefHost = new ListPreference(this);
        ...
        if (prefHost != null) {
            List<ServerInfo> servers = app.serverDiscovery.servers;
            CharSequence entries[] = new String[servers.size()];
            CharSequence entryValues[] = new String[app.serverDiscovery.servers.size()];
            int i = 0;
            for (ServerInfo server : servers) {
                entries[i] = server.name + " ("+server.address+")";
                entryValues[i] = server.address;
                i++;
            }
            prefHost.setEntries(entries);
            prefHost.setEntryValues(entryValues);
        }
